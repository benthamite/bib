...
#+export_file_name: bib.info
#+texinfo_filename: bib.info
#+texinfo_dir_category: Emacs misc features
#+texinfo_dir_title: Bib: (bib)
#+texinfo_dir_desc: Retrieve bibliographic metadata
...
The development repository is on [[https://github.com/benthamite/bib][GitHub]].
...
| =bib-search-imdb=                 | Search OMDb by movie title, including non-ASCII titles, and get the full IMDb URL |
...
Note: =bib-search-isbn= now uses [[https://openlibrary.org][Open Library]] (free, no key required).  The =bib-isbndb-key= variable is obsolete.
...
The test suite uses ERT.  Run it locally with:
...
CI runs both steps on Emacs 29.4 and 30.1 via GitHub Actions.
